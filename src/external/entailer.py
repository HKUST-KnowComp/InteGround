# This code is from the EntailmentBank official repo: 
# https://github.com/allenai/entailment_bank/blob/main/utils/nlp_agent.py
...
def init_entailer(model="allenai/entailer-large", device='cuda:0'):
    global prover

    if device.startswith("cuda"):
        ew_model = MultiAngleModel(model_path=model, cuda_devices=device[-1])
    else:
        ew_model = MultiAngleModel(model_path=model)
    ew_model.name_or_path = model

    prover = NlpAgent(model=ew_model, default_outputs="proof")

# def entailer_generate(input_text, model="allenai/entailer-large", device='cuda:0'):
#     global prover

#     if 'prover' not in globals():
#         print("Initing model:", model)
#         init_entailer(model=model, device=device)
        
#     elif globals()['prover'].model.name_or_path != model:
#         del prover
#         torch.cuda.empty_cache()
#         print("Initing model:", model)
#         init_entailer(model=model, device=device)

#     # return json.loads(propositionizer(input_text)[0]['generated_text'])
#     proof = prover({"hypothesis": input_text})
...
entailer_template = \
"""Given the following hypothesis, try to generate a set of premises that can prove the hypothesis. 
Please format the premises as {{"Premises": ["PREMISE 1 TEXT", "PREMISE 2 TEXT", ...]}}.
...
{{"Premises": ["Leo is a kind of constellation.", "A constellation contains stars.", "The earth revolving around the sun causes stars to appear in different areas in the sky at different times of year."]}}

Hypothesis: The earth rotating on its axis causes stars to move relative to the horizon during the night.
{{"Premises": ["Apparent motion is when an object appears to move relative to another object's position.", "The earth rotating on its axis causes stars to appear to move across the sky at night.", "Earth is a kind of celestial object.", "A star is a kind of celestial object / celestial body.", "Stars appear to move relative to the horizon during the night."]}}

Hypothesis: {}"""

def entailer_generate(input_text, model="allenai/entailer-large", device='cuda:0'):
    if model.startswith("allenai/entailer"):
        global prover
        ...
        while retry < 3:
            try:
                res = get_llm_response(entailer_template.format(input_text), model=model)
                premises = eval(get_dict(res))['Premises']
                break
            except Exception as e:
                print("[In entailer generate]", e, retry)
                print(res)
                retry += 1
